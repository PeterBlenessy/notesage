 ...
 *  - Tag/mention badge click → command bar (Quiet Composer only)
...
export function useAppLifecycle() {
  ...
  // In-document `#tag` clicks fire `notesage:open-tag-search`. The
  // FloatingCommandBar subscribes and drills straight into TagMode's
  // level-2 occurrences view — same drilldown shape the sidebar
  // `TagsSection` already uses (audit #1, sidebar-simplification task #17).
  useEffect(() => {
