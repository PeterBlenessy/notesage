...
/**
 * Vertical drag handle on the left edge of the pinned panel.
 * The actual width state lives in the `--cmd-bar-pinned-width` CSS variable on
 * <html>; we only persist the final value to settings-store on pointerup / keyup.
 * This keeps mousemove paths free of React re-renders.
 */
export function PinnedResizeHandle() {
  ...
  const onKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const next = Math.max(
        PINNED_WIDTH_MIN,
        Math.min(PINNED_WIDTH_MAX, persistedWidth + delta),
      );
      document.documentElement.style.setProperty(
        '--cmd-bar-pinned-width',
        `${next}px`,
