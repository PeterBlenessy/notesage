    ...
    (set) => ({
      ...
      openTab: (filePath: string, fileName: string, content: string, frontmatter?: Frontmatter | null, fileType?: FileType, scrollToTag?: ScrollToTag, scrollToText?: string) => {
        set((state) => {
          ...
          // as closeTab today (it doesn't gate on dirty either; the
          // warn-if-dirty path is in TitleBar at the call site, not here).
          ...
          // Create new tab. Evict every other open doc so the end state
          // is always exactly 1 entry (Quiet Composer is single-doc).
          const baseDocs: typeof state.openDocuments = [];
      ...
      closeTab: (tabId: string) => {
        set((state) => {
          ...
          // If closing the active tab, land on the empty state. Quiet
          // Composer is a single-document shell by design (see openTab
          // eviction above), so closing always returns to the empty state.
