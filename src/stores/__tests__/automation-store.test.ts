...
describe('automation-store', () => {
  ...
  it('recordRun prepends, dedupes by runId, and caps at 20', () => {
    const store = useAutomationStore.getState();
    for (let i = 0; i < 25; i++) {
      store.recordRun(run({ runId: `r${i}`, startedAt: Date.now() }));
    }
    // re-record an existing id — should not duplicate
    store.recordRun(run({ runId: 'r24', startedAt: Date.now() }));
