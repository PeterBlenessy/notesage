...
interface SettingsStore {
  ...
  /**
   * Anonymous, randomly generated install identifier. Not tied to any account
   * or device fingerprint — used only to deduplicate DAU/MAU counts. Generated
   * once on first run and regenerated by `resetTelemetryInstallId`.
   */
  ...
  /** Regenerate a fresh random install identifier. */
...
function applyTelemetryConsent(state: SettingsStore): void {
  try {
    invoke('telemetry_apply_consent', {
      ...
      /* swallow — telemetry sync is non-critical */
    });
  } catch {
  ...
  persist(
    (set, get) => ({
      ...
      resetTelemetryInstallId: () => {
        set({ telemetryInstallId: crypto.randomUUID() });
      },
