/**
 * Markdown ↔ HTML converter bank — the pure-ish preprocessor/postprocessor
 * helpers that run around the Tiptap markdown round-trip.
 *
 * Extracted verbatim from `markdown.ts` (behavior-preserving refactor) so the
 * round-trip contract (`getMarkdownFromEditor`, `setMarkdownInEditor`,
 * `loadRawMarkdownIntoEditor`, `loadParsedJsonIntoEditor`, `streamingHydrate`,
 * `prepareInitialContent`) stays readable in isolation over there.
 *
 * Everything here is re-exported from `@/lib/markdown` for compatibility —
 * existing import paths keep resolving. Contents:
 *
 *   - `convert*ToHtml` preprocessors (callouts, link previews, drawings,
 *     charts, mermaid, inline fenced blocks, data-URI images, images with
 *     block metadata, TOC, page breaks)
 *   - `restore*` postprocessors (TOC, page breaks)
 *   - `strip*` / `inject*` / `apply*` side-channel families (annotations,
 *     node IDs, table column metadata, ghost task items)
 *   - image path space encoding/decoding
 *
 * Worker-safety note: the pure string→string transforms here are consumed by
 * the markdown-parse Web Worker via the curated `markdown-preprocess-pure.ts`
 * re-export. This module's only import is the type-only `Editor` — no DOM,
 * no Tauri IPC, no editor-storage — so it stays worker-bundle-safe. The
 * `apply*ToEditor` / `inject*` helpers take an `Editor` at runtime and are
 * main-thread-only by construction (they are not re-exported to the worker).
 */
