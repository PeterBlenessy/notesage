...
// Round-trip test for #173 follow-up: width/align settings on chart, drawing,
// link-preview and image nodes must persist across save → reopen, including
// for legacy sidecar references that haven't been auto-migrated to inline form.
//
// The serialiser writes a trailing `<!--blockWidth:N,align:X-->` HTML comment
// after the sidecar image; the parser reads it back into:
//   - `data-block-width="N"` (consumed by node `parseHTML`)
//   - `style="text-align: X"` (consumed by TextAlign extension's default
//     `parseHTML` which reads `element.style.textAlign`).
