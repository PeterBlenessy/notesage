...
describe("worker-extensions schema fingerprint", () => {
  it("matches the inline snapshot — bump CACHE_SCHEMA_VERSION before updating", () => {
    ...
    expect(fingerprint).toMatchInlineSnapshot(
      `"2|callout|chart|codeBlock|color|drawing|heading|highlight|image|linkPreview|mermaidBlock|pageBreak|paragraph|starterKit|subscript|superscript|table|tableCell|tableHeader|tableOfContents|tableRow|taskItem|taskList|textAlign|textStyle|uniqueID"`,
    );
  });
});
