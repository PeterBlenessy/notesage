...
class View {
    ...
    setImageSize() {
        ...
        // Constrain tables to the column width to prevent horizontal overflow
        // into adjacent pages. Unlike images, tables are NOT given break-inside:
        // avoid — tall tables must be allowed to span page breaks naturally.
        if (columnWidth) {
            const maxW = vertical
                ? `${width - margin * 2}px`
                : `${Math.trunc(columnWidth)}px`
