    ...
    let rows = stmt
        .query_map(param_refs.as_slice(), |row| {
            Ok((
                ...
                row.get::<_, String>(3)?,
                row.get::<_, String>(4)?,
                row.get::<_, i64>(5).map(|v| v as usize)?,
    ...
    let rows = stmt
        .query_map(param_refs.as_slice(), |row| {
            ...
            Ok(IndexedTask {
                ...
                position: row.get::<_, i64>(4).map(|v| v as usize)?,
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            Ok(IndexedGoal {
                ...
                title: row.get(2)?,
                template: row.get(3)?,
                total_tasks: row.get::<_, i64>(4).map(|v| v as usize)?,
                completed_tasks: row.get::<_, i64>(5).map(|v| v as usize)?,
                project_name,
            })
        })
        .map_err(|e| e.to_string())?;
