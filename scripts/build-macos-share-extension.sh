...
# nothing to embed into. So this runs against an UNSIGNED bundle and signs the
# whole thing afterwards — inside out, extension first, app last, which is what
# codesign requires for nested code.
set -euo pipefail
...
#
# The same crate the iOS extension links. Building it per-arch and lipo-ing
# keeps the extension universal like the app.
