---
name: aw-triage
description: Classify a fresh top-level GitHub issue (bug / enhancement / chore / duplicate / wontfix), set the category label, mark it as `feature` (parent), and add the `clarify` action label so aw-clarify picks it up next.
---

# aw-triage

Classify a single top-level GitHub issue and set its category label. Don't modify the body — that is `aw-clarify`'s job.
...
3. **Decide a status.** Exactly one of:
   ...
   - **Genuinely ambiguous** (one-line title, no body, mixes unrelated topics) → post a clarification comment, do NOT add a category label, do NOT add `clarify`, do NOT add `feature`. Stop.
...
5. **Apply labels and comment.**
   ...
   - Add the `clarify` action label (signals `aw-clarify` to pick this up next).
...
- **Exactly one** action label after a successful run: `clarify`. Never set `slice`, `sliced`, `tdd`, or `review` here.
...
In all of these: post the clarification template, do not add a category, do not add `feature` or `clarify`. The issue stays untriaged.
...
- After a successful run: issue has `<category>` + `feature` + `clarify` (plus any pre-existing area labels).
- `feature` ≠ category. `feature` is the "this is a top-level issue" marker, set on all parents regardless of whether they're bugs, enhancements, or chores. `enhancement` is the category for "new functionality / improvement". They co-exist on a feature-request issue: `enhancement + feature + clarify`.
