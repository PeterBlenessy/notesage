...
5. **The same applies to TESTS, not just renderers.** Behaviour assertions live in three places with different runners: unit/component specs, the Playwright suite under `e2e/`, and the WebDriver suite under `e2e-real/`. The unit runner and the typechecker both EXCLUDE `e2e/` and `e2e-real/`, so specs there pass locally by *not running* and only fail in CI. When changing app-wide behaviour (keyboard shortcuts, UI chrome), grep those directories too. Concrete recurrence: removing a keyboard shortcut left 5500+ unit tests green locally while five specs across both E2E suites, plus a JSDoc block, still drove the old binding — two CI jobs red, and a push→CI→fix round trip a single pre-push grep would have avoided.
