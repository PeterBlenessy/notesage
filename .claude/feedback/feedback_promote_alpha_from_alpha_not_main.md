...
- **Land the stable history entry on the default branch promptly** (a small docs PR), ideally as part of the promotion and BEFORE the next automated alpha cut runs. The promotion tags from a release branch, so the stable `docs/history/NNN-release-vX.Y.Z.md`, the README row and the regenerated `public/changelog.json` do NOT reach the default branch on their own. Two failure modes if this is skipped: (1) the next alpha's history file is numbered from the default branch and **reuses the number** the stable entry took on the release branch; (2) future builds regenerate the changelog from the default branch's `docs/history`, so the stable "What's new" feed **silently omits the version just shipped**.
