...
Stable promotion of the `0.47.0-alpha.1 … 8` line, tagged at the alpha.8 commit so the release contains only alpha-tested code; the dev-only dependency bumps (#489) were excluded and ship on `0.48.0-alpha.1`.

This history entry lands on `main` after the fact: the promotion was tagged from a release branch, and the next auto-cut (`v0.48.0-alpha.1`) reused history number 165 before this stable entry reached `main` — hence the slightly out-of-order numbering (166 follows the alpha's 165; the dates are authoritative). The shipped `v0.47.0` tag/release is unaffected.

Marquee work:

- **Command-bar session lifecycle & concurrent multitasking** (#469) — per-conversation ACP agent registry, run-state store, streaming writes, tool-permission ownership with foreground-aware auto-deny, a concurrency cap + FIFO queue (`maxConcurrentSessions`), history-row status badges + switcher, inline permission cards in history rows, orb unwatched-session list + needs-you pulse, and desktop notifications for backgrounded sessions. Deferred item #11 tracked in issue #468.
...
Stable release notes for v0.47.0, added to `main` retroactively: this history entry, a README index row, and the regenerated `public/changelog.json` so the stable "What's new" feed includes v0.47.0.
