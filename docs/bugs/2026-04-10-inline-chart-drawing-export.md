...
Avoids: hash matching, filesystem caching, timing issues. Risk: Positional matching between frontend node order and comrak AST walk order must be reliable.
...
Avoids: SVG handling complexity, Typst virtual filesystem. Risk: Image quality (rasterization), large markdown strings, data URI support in each exporter.
...
Avoids: All frontend-backend coordination issues. Risk: Large implementation effort, feature parity with recharts (10 chart types), maintaining two rendering paths.
