...
**Current:** `com.notesage.app`**Required:** Must match Apple Developer Team prefix, e.g., `com.peterblenessy.notesage`**Location:** `src-tauri/tauri.conf.json` → `identifier`
...
**Name:** Notesage **Subtitle:** AI-Powered Markdown Editor **Category:** Productivity **Age rating:** 4+ (no objectionable content)
...
**Scope:** Migrate API keys from plaintext localStorage to macOS Keychain **Key deliverables:**
...
**Scope:** Close all security gaps found in audit **Key deliverables:**
...
**Scope:** Everything needed to submit to the Mac App Store **Key deliverables:**
