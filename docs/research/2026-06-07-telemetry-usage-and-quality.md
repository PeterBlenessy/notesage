...
**Consent (channel-based):** Given the privacy-first brand, the default is tied to the existing **release channel** (`settings.releaseChannel`) — **alpha defaults on, stable defaults off (opt-in)** — exposed as a Settings → System toggle, co-located with the Stable/Alpha selector, that the user can flip either way. The channel only sets the toggle's initial value; the toggle itself is the single opt-out mechanism (no separate env/kill-switch concept). The disclosure appears right at the channel selector (choosing Alpha is the consent moment), keeping this on the right side of VS Code's much-criticized opt-out default. Obsidian's "collect nothing" stance is the bar Notesage is implicitly measured against, so transparency and an honest default matter more here than for a typical app.
...
The naive "opt-in, default off everywhere" posture has a fatal flaw for this use case: opt-in telemetry gets **<5% participation**, so for an alpha audience it would yield almost no data — defeating the purpose. The decision is to tie the default to the **release channel** — which already exists as a user setting (`settings.releaseChannel`, the Stable/Alpha selector in Settings → System) — treating alpha users as testers in an explicit value exchange (bleeding-edge features ↔ usage/crash signal), while keeping stable conservative:

| Channel | Usage analytics | Crash reports | Opt out via |
| --- | --- | --- | --- |
| **Alpha** | Default **ON** | Default **ON** (PII-scrubbed) | The Settings → System toggle (beside Release channel) |
| **Stable** | Opt-in (default **OFF**) | Opt-in (default **OFF**) | The Settings → System toggle (already off) |
...
2. **Disclose at channel selection (primary surface).** The Stable/Alpha selector already lives in **Settings → System** beside "Check for updates." Choosing Alpha *is* the moment the user accepts default-on telemetry, so the disclosure belongs right there: an inline note under the selector while Alpha is active — "Alpha builds share anonymous usage + crash reports by default to help stabilize fast-moving features. No document content, file contents, or AI prompts are ever sent." — plus a one-time confirming toast on a stable → alpha switch. Wired to the existing `setReleaseChannel` handler.
3. **First-run notice** for installs that start on alpha and haven't seen it: the same message once, as a non-blocking toast with an "Open settings" action. Transparency is what converts "sneaky" into "fair."
4. **Telemetry toggles in Settings → System** (co-located with Release channel + updates — the default *is* a property of the channel choice, so the controls belong together): two independent switches — **Usage analytics** and **Crash reports** — so a user can keep crash reports (helps fix *their* bugs) without usage tracking, or neither.
5. **Persist in `settings-store`** as tri-state `telemetryUsageEnabled` / `telemetryCrashEnabled` (`null` = follow channel, `true`/`false` = explicit override). Effective value: `flag ?? (releaseChannel === 'alpha')`. Gate every `trackEvent` / SDK init on the effective value; if a stream is off, don't initialize that SDK at all. The channel is the **existing `settings.releaseChannel`** — no version-string parsing or new detection command needed.
...
**Implementation note:** both channels already exist — `settings.releaseChannel` (`'stable' | 'alpha'`, default `'stable'`) is the live selector in `SystemSettings.tsx`, consumed by `useAutoUpdate`/`alpha_check`. The telemetry default keys off this value directly, and the disclosure hooks the existing `setReleaseChannel` handler, so the consent flow is wireable today with no new channel-detection machinery.
...
**Phase 3 — Consent UX (channel-based).** Derive the default from `settings.releaseChannel` — alpha **on**, stable **off** — with two independent Settings → System toggles (usage / crashes) as the single opt-out, co-located with the Release channel selector. Disclose at the selector (and a one-time notice on stable → alpha). Ship this *with* Phase 1/2, not after, and ensure crash-report PII scrubbing is in place before crash reporting goes default-on.
