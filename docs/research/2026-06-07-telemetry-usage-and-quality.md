...
2. **Honest first-run notice** (alpha): a one-time non-blocking banner/toast — "Alpha builds share anonymous usage and crash reports to stabilize fast-moving features. This is on by default; turn it off in Settings > Privacy. No document content, file contents, or AI prompts are ever sent." Transparency is what converts "sneaky" into "fair."
   - **Disclose at channel selection too.** The same message must appear wherever the user opts into the alpha channel — choosing alpha *is* the moment they accept default-on telemetry, so the disclosure belongs at that decision point, not only at first launch. Today alpha is the only channel (so the first-run notice carries it); when an explicit release-channel picker exists, show an inline note beside it and re-surface the notice on stable → alpha switches.
