...
The cross-project link graph lives in a single workspace-level store
(`~/.notesage/links.db`, or a dedicated table set in the global index db) that
records every document-to-document edge across all projects in one place, keyed
by source/target paths plus the resolved target file-id when known. The
per-project `<project>/.notesage/index.db` databases stay focused on
within-project content (tags, mentions, tasks, goals, FTS); the cross-cutting
graph is centralized.
