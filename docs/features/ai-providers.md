...
- **Local Agent preset — pi engine (`localAgentPreset: 'pi'`, PRD `docs/prds/2026-07-29-pi-local-agent-preset.md`):** a *second* Local Agent engine offered beside Goose (Goose stays the default; the setup dialog has an engine picker). pi ([earendil-works/pi](https://github.com/earendil-works/pi), MIT) has no ACP, no MCP, and no permission prompts in core — its embedding surface is a JSONL RPC over stdio plus TypeScript extensions — so Notesage supplies those layers itself and the rest of the ACP stack (session updates, PermissionCards, segments, smoke test, session restore) is unchanged. Three pieces make it work:
  - **`notesage-acp-pi` bridge** ([`PeterBlenessy/notesage-acp-pi`](https://github.com/PeterBlenessy/notesage-acp-pi) — its own repository and version line, which is what makes the exact pin in `agent_manager.rs` possible; Bun-compiled to a self-contained per-platform binary, installed checksum-verified from that repo's releases). It presents ACP (via `@agentclientprotocol/sdk`) on its stdio and drives `pi --mode rpc` as a child in its own process group (awaited group-kill teardown so `kill_on_drop` can't orphan pi). It translates pi events → ACP session updates (text/thinking chunks, `tool_call`/`tool_call_update` with Diff content derived from edit/write args, `usage_update` from `get_session_stats`), maps ACP session/new·load·fork·prompt·cancel onto pi's `new_session`/`switch_session`/`clone`/`prompt`/`abort` (ACP sessionId = pi's session **file path**; ACP fork = pi `clone`), and brokers `extension_ui_request` ↔ ACP `session/request_permission`. **Load-bearing rule (spike #3):** the broker answers a pending UI request (`cancelled: true`) *before* any `abort` — aborting with an open request wedges pi. The connection spawns the bridge as `notesage-acp-pi --pi-bin <pi> -- --provider local --model <id> --session-dir <dir>` (the post-`--` args regenerated per endpoint so a model switch respawns with fresh selection).
  - **Two Notesage-shipped pi extensions**, embedded at compile time and (re)written into the flat `PI_CODING_AGENT_DIR` (`~/.notesage/agents/pi/`) on every config generation: a **permission gate** (blocks non-read-only tool calls behind `ctx.ui.select`, block-by-default with no UI; stateless — tiered approvals stay in Notesage's `ScopedApproval` store) and an **MCP tools** extension (reads stdio server configs from the `NOTESAGE_MCP_SERVERS` env — resolved keychain secrets ride the env, never disk — spawns each server under pi's sandbox, and registers its tools).
  - **Config (`local_agent_write_config(agent: 'pi')`)** writes `models.json` (dummy-key OpenAI-compatible provider at the live llama-server port), `settings.json` (`enableInstallTelemetry: false`), and the two extensions; the spawn env is `PI_OFFLINE=1` + `PI_CODING_AGENT_DIR` + `NO_PROXY` (defense-in-depth — pi ignores proxy vars). Same `<port>:<modelId>` respawn key and the same empty-allowlist / kernel-deny / llama-port-allowed sandbox posture as Goose (zero Bucket C `$HOME` grants — the whole footprint is under the `.notesage` write-allow). Install goes through the managed-install registry with **checksum verification** (pi against its `SHA256SUMS`, the bridge against `notesage-acp-pi-SHA256SUMS`) and an **exact-tested version pin** (pi is 0.x with multiple releases/week; installs clamp to the pin and newer upstream releases surface as "held back" in `agent_check_updates` until a Notesage release moves it). **Status: implemented; the Seatbelt zero-network spike and the live end-to-end verification are macOS-execution gates pending before merge.**
...
- **Agent slash commands**: `available_commands_update` events populate the `/` command menu alongside Notesage skills (`SkillMode` reads them from session state; Terminal icon vs the Sparkles used for skills). Skills are listed first so an agent command can never displace the skill a user was reaching for. Picking one inserts `/name` into the composer — `expandSkillPrefix` leaves an unmatched name untouched, so the text reaches the agent verbatim on send. This is the only compaction control available for an ACP agent: the agent owns its own context window, and the protocol exposes neither a compaction request nor a notification when the agent compacts on its own (ACP 0.11.4 has no such session update; `session/fork` is the protocol's blessed way to summarize without disturbing a session's history).
...
| `src-tauri/src/commands/local_agent.rs` | Local Agent preset: `local_agent_write_config(agent)` — Goose env (provider + host + model + isolated XDG tree, no config file) OR pi flat config tree (models.json + settings.json + shipped extensions + `PI_OFFLINE`/`PI_CODING_AGENT_DIR` env + piArgs) |
| `bridges/pi-acp/extensions/` | The two pi extensions Notesage ships into the pi config dir (permission-gate, mcp-tools), **vendored** from the upstream bridge repo and embedded via `include_str!`. The bridge itself lives at [`PeterBlenessy/notesage-acp-pi`](https://github.com/PeterBlenessy/notesage-acp-pi) — fix bugs there, then re-vendor and move the pin. See `bridges/pi-acp/README.md`. |
