...
| `bridges/pi-acp/` | The `notesage-acp-pi` ACP↔pi-RPC bridge (in-repo TypeScript, Bun-compiled): `src/` (pi-process, pi-rpc, acp-server, translate, permissions, mcp-handoff, usage) + `extensions/` (permission-gate, mcp-tools shipped into the pi config dir) |
