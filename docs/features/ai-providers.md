...
- **Local Agent preset — pi engine (`localAgentPreset: 'pi'`, PRD `docs/prds/2026-07-29-pi-local-agent-preset.md`):** a *second* Local Agent engine offered beside Goose (Goose stays the default; the setup dialog has an engine picker). pi ([earendil-works/pi](https://github.com/earendil-works/pi), MIT) has no ACP, no MCP, and no permission prompts in core — its embedding surface is a JSONL RPC over stdio plus TypeScript extensions — so Notesage supplies those layers itself and the rest of the ACP stack (session updates, PermissionCards, segments, smoke test, session restore) is unchanged. Three pieces make it work:
  - **`notesage-pi-acp` bridge** (`bridges/pi-acp/`, in-repo TypeScript, Bun-compiled to a self-contained per-platform binary published on Notesage's own releases). It presents ACP (via `@agentclientprotocol/sdk`) on its stdio and drives `pi --mode rpc` as a child in its own process group (awaited group-kill teardown so `kill_on_drop` can't orphan pi). It translates pi events → ACP session updates (text/thinking chunks, `tool_call`/`tool_call_update` with Diff content derived from edit/write args, `usage_update` from `get_session_stats`), maps ACP session/new·load·fork·prompt·cancel onto pi's `new_session`/`switch_session`/`clone`/`prompt`/`abort` (ACP sessionId = pi's session **file path**; ACP fork = pi `clone`), and brokers `extension_ui_request` ↔ ACP `session/request_permission`. **Load-bearing rule (spike #3):** the broker answers a pending UI request (`cancelled: true`) *before* any `abort` — aborting with an open request wedges pi. The connection spawns the bridge as `notesage-pi-acp --pi-bin <pi> -- --provider local --model <id> --session-dir <dir>` (the post-`--` args regenerated per endpoint so a model switch respawns with fresh selection).
...
| `bridges/pi-acp/` | The `notesage-pi-acp` ACP↔pi-RPC bridge (in-repo TypeScript, Bun-compiled): `src/` (pi-process, pi-rpc, acp-server, translate, permissions, mcp-handoff, usage) + `extensions/` (permission-gate, mcp-tools shipped into the pi config dir) |
