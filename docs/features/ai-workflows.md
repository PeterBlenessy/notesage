...
**The artifact bundle:** each recording is a folder under the inbox (and later the chosen project) holding the finalized capture (`audio.wav` from the Mac, `audio.m4a` from the phone) + `recording.json` (the manifest, below) + `transcript.md` (note rendered from segments). The folder keeps them together so "move to project" is a single atomic move (`src/lib/transcription/bundle.ts`, reusing `rename_path` / `copy_directory`); every helper there takes only `dirname` / `basename` of the audio path, so the container never matters.

**The manifest — `recording.json` (PRD `2026-09-05-ios-recordings.md`):** the contract between whoever recorded a bundle and the Mac that transcribes it, shared with the phone through one fixture (`tests/fixtures/recording-manifest.v1.json`, decoded by both the vitest and the Swift `Codable` test). `src/lib/transcription/manifest.ts` owns it: `RecordingManifest` (`version: 1`, `createdBy { device, app, appVersion }`, `startedAt` with offset, pause-aware `durationSecs`, `language?`, `audio { file, bytes, codec: aac | pcm, sampleRate, channels, bitrate? }`, `transcription`), `parseRecordingManifest` (strict on the fields it knows, tolerant of the ones it does not — unknown fields ride through a rewrite), `serializeRecordingManifest` (contract key order, two-space indent, trailing newline — byte-stable), `isPendingTranscription`, `withTranscriptionStatus`. The Mac writes the same manifest for its own bundles right after `stop_recording` (`useMeetingRecording.writeMacRecordingManifest`: `app: "notesage-macos"`, `codec: "pcm"`, `bytes` from `file_size`, `device` from `get_device_name`), so the format is bilateral and the scanner treats both kinds of bundle by one rule.

**The pending rule and discovery:** a bundle is *pending* when `recording.json` exists and `transcript.md` does not. `useRecordingsInbox` (`src/hooks/useRecordingsInbox.ts`, mounted in `App.tsx` beside `useTranscriptionJob` — an unmounted hook never runs) lists `<library root>/Recordings` on `startupReady` (the Inbox's root rule: `settings.icloudNotesagePath` when sync is on, else `resolveNotesRoot`; `recordingsDir` in `notes-root.ts`) and re-evaluates any bundle a `file-changed-batch` `create` / `modify` touches (debounced 2 s per bundle). The pure `evaluateBundle` turns pending into eligible: not `done`, not `failed` (the card's Re-run is the retry — a bundle the Mac cannot decode must not burn a Whisper run every launch), not `running` on *another* device within the last hour, not already tracked in `activity-store` by `audioPath`, and `file_size(audio) === audio.bytes` (a truncated `.m4a` decodes silently to a truncated transcript; an evicted iCloud placeholder triggers `icloud_ensure_downloaded` and the scanner waits for the watcher event the arriving file produces — no polling). Eligible bundles enter a FIFO in stamp order and are dispatched to `startTranscription` **one at a time**. The claim — `transcription: { status: "running", device, updatedAt }` — is written into the manifest before the dispatch; `done` (with `model` and the detected `language`) and `failed` (with the `error`) are written from the activity-store transition, which also covers the Mac's own recordings and Re-runs, so the phone's recording row always shows the latest state. The `TranscriptionCard` captions a phone bundle's job *from Peter's iPhone* (`AgentTask.sourceDevice`). Mac bundles from before the manifest existed have no `recording.json` and are left alone.
...
| `src/hooks/useRecordingsInbox.ts` | Recordings scanner (mounted in `App.tsx`) — lists `<library root>/Recordings`, gates on the manifest's byte count, dispatches phone bundles one at a time, writes claim / done / failed back into `recording.json` |
| `src/lib/transcription/manifest.ts` | The `recording.json` contract — parse (tolerant), serialize (byte-stable key order), pending rule, status rewrite |
