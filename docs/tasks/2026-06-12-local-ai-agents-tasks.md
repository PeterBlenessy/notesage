...
**Done (2026-06-13, on macOS):** `pnpm typecheck` clean; `pnpm test` 5523 pass / 0 fail (324 files); `pnpm test:e2e` Local Agent happy-path green (chromium + webkit); `cargo test --lib` 843 pass + `cargo test --test sandbox_isolation -- --ignored` (real Seatbelt: `leak_6c`, `leak_6d`, `sandbox_sentinel`) 3 pass; `tauri-capability-surface.test.ts` green (part of the 5523); `pnpm coverage:check` 0 regressions (70.22%). `pnpm test:perf` — the `orb.perf` benchmark flaked once under full-run contention but passes 5/5 in isolation; pre-existing/unrelated to this feature, and the frontend perf step is advisory on CI. Feature is lazy/off all hot paths → no perf delta, no baseline entry recorded (contended numbers would pollute history).
**Live macOS run (2026-06-13):** npm install validated end-to-end (opencode-ai@1.17.4 via npm, binary runs, symlinks correct); opencode↔bundled-llama-server↔gemma-4 completes a full agentic turn. **Found + fixed a real bug:** the preset started the server with the store chat default `contextLength=4096`, but OpenCode's agentic system prompt is ~7.3K tokens → every agentic turn (and the smoke test) failed with "request exceeds the available context size". Fix: `resolveLocalAgentContext` floors the agent server's context at `LOCAL_AGENT_MIN_CONTEXT` (16384) in `ensureServerRunning` (`src/lib/ai/local-agent-model.ts` + unit test; `src/hooks/useLocalAgentSetup.ts`). Confirmed: 4096 fails, 16384 returns a clean turn. A manual `e2e-real/tests/local-agent-setup.manual.ts` spec exists as the end-to-end companion — named `.manual.ts` (not `.test.ts`) so the CI Real-E2E globs skip it (it needs network + a downloaded model + a clean single instance the runner can't provide); run it by hand. The WKWebView WebDriver harness is finicky for this flow (see the spec header); the unit test is the deterministic regression lock.

---

## Addendum (2026-06-13): preset agent changed OpenCode → Goose

The Local Agent preset binary was changed from **OpenCode** to **Goose** (Block's open-source ACP agent, `block/goose`) after OpenCode proved incompatible with the strict Seatbelt sandbox. This addendum records the change without rewriting the task history above — the OpenCode-specific details in #7/#8/#9 reflect the original implementation and the live-run notes below #21/#23 reflect the OpenCode bug found at the time.

**Why the switch:** OpenCode bootstrap requires (a) an npm install of its provider SDK (`@ai-sdk/openai-compatible`) during the smoke test and (b) a cloud model-registry fetch (models.dev) + autoupdate manifest at `session/new`. Even with `OPENCODE_DISABLE_MODELS_FETCH` / `OPENCODE_DISABLE_AUTOUPDATE` and a setup-only npm-registry allowlist exception, this could not be reconciled with the goal of a **100% empty network allowlist** under `(deny default)`. Goose, by contrast, is a self-contained Rust binary that needs no Node runtime, no npm install, and no cloud auth, and was empirically verified to complete an agentic turn under the strict sandbox with zero network and **no config file**.

**What changed (this batch):**

- **`#7` install:** OpenCode (npm `opencode-ai`, min 1.17.4) → Goose via the **GitHub-release-binary** install path (`block/goose`, asset `goose-{rust-target-triple}.tar.gz` — `aarch64-apple-darwin` / `x86_64-apple-darwin`, min version 1.37.0). The GitHub-binary install machinery (deleted when OpenCode moved to npm) was restored and adapted for Goose. `agent_manager.rs`.
- **`#8` config:** OpenCode `opencode.json` generation → **env-only** Goose configuration (no file written): `GOOSE_PROVIDER=openai`, `OPENAI_HOST=http://localhost:<port>`, `GOOSE_MODEL=<id>`, dummy `OPENAI_API_KEY`, `GOOSE_DISABLE_KEYRING=1`, and all four `XDG_*` dirs redirected into `~/.notesage/agents/goose`. `local_agent.rs` (`local_agent_write_config` keeps the same command name + `LocalAgentConfig` shape).
- **`#9` sandbox:** Bucket C fallback dirs changed from `~/.config|.cache|.local/share/opencode` to the four Goose XDG dirs (`~/.config/goose`, `~/.cache/goose`, `~/.local/share/goose`, `~/.local/state/goose`). The smoke test now uses an **empty** `networkAllowedDomains` (no npm-registry exception) and `agentArgs: ['acp']`. `sandbox.rs`, `useLocalAgentSetup.ts`.
- **`localAgentPreset` discriminator:** `'opencode'` → `'goose'` across `connections.ts` and every comparison/test site.
- **`#22` docs:** `docs/features/ai-providers.md` Local Agent preset section rewritten for Goose (env-based, no config file, no network, `goose acp`) with attribution.
- **Attribution:** added to the setup dialog and the Add-Connection Local Agent card ("powered by Goose, an open-source agent by Block").
