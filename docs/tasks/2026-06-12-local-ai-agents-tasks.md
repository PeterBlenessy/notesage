...
**Live macOS run (2026-06-13):** npm install validated end-to-end (opencode-ai@1.17.4 via npm, binary runs, symlinks correct); opencode↔bundled-llama-server↔gemma-4 completes a full agentic turn. **Found + fixed a real bug:** the preset started the server with the store chat default `contextLength=4096`, but OpenCode's agentic system prompt is ~7.3K tokens → every agentic turn (and the smoke test) failed with "request exceeds the available context size". Fix: `resolveLocalAgentContext` floors the agent server's context at `LOCAL_AGENT_MIN_CONTEXT` (16384) in `ensureServerRunning` (`src/lib/ai/local-agent-model.ts` + unit test; `src/hooks/useLocalAgentSetup.ts`). Confirmed: 4096 fails, 16384 returns a clean turn. A manual `e2e-real/tests/local-agent-setup.test.ts` spec exists as the end-to-end companion (the WKWebView WebDriver harness is finicky for this flow — see the spec header; the unit test is the deterministic regression lock).
