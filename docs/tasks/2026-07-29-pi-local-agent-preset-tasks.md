...
### #23 — Docs + quality-gates sweep ✅ (Linux gates all green: root typecheck, root vitest 6476 passed/0 failed, bridge typecheck + 51 tests, cargo check, capability-surface unchanged, node24 lint. Docs updated. Fixed two self-introduced issues found by the sweep: spike-workflow lint (checkout@v6 + upload-artifact TODO) and root vitest globbing bridges/node_modules. macOS-only gates in #23's checklist — real-Seatbelt cargo test, e2e — run in CI/on #22.)
