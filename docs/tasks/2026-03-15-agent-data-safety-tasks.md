...
**Implementation order:** Backend sandbox updates first (tasks #1-3), then chat store/session infrastructure (#4-5), then UI (#6-9), then delegation integration (#10), and finally workspace respawn (#11).

**Risks:**
- Task #5 (message filtering by segment) touches the core chat send path — high blast radius, test carefully
- Task #6 (project switch prompt) introduces a new message type in the chat — needs careful integration with scroll, persistence, and rendering
...
- **Description:** Create `ContextDivider` component. Collapsible horizontal rule with context label. Collapsed (default): `▸ Context: Project Name ────`. Expanded: shows switched-from, history status, session status. Render after each resolved `ProjectSwitchCard` in the message list. Read segment data from chat store to populate details.
...
- **Description:** Update the chat message rendering loop in `ChatPanel.tsx` to interleave `ProjectSwitchCard` (pending) and `ContextDivider` (resolved) between message bubbles at segment boundaries. Messages from prior segments render normally (no hiding). The divider visually separates contexts. Ensure scroll-to-bottom still works after inserting cards. Ensure cards persist across re-renders and are restored from conversation state on app restart.
- **Files:** `src/components/chat/ChatPanel.tsx`
...
- **Complexity:** M | **Category:** frontend | **Depends on:** #2
- **Description:** In `useCommentDelegation.ts`, resolve the document's parent project or explorer folder from workspace-store (using `getExplorerFolderForFile()` or matching against `projects`). Pass this single folder as `sandboxPaths: [parentFolder]` when spawning the task agent. If no parent folder found (file outside workspace), fall back to the file's directory. Verify inline actions in `useAcpLifecycle.ts` `acpGenerateText` also use single-folder sandboxing.
- **Files:** `src/hooks/useCommentDelegation.ts`, `src/hooks/useAgentTaskOperations.ts`, `src/hooks/useAcpLifecycle.ts`

### #11 — Respawn agent on workspace changes

- **Complexity:** M | **Category:** frontend | **Depends on:** #3
- **Description:** Watch workspace-store for changes to `explorerFolders` and `projects` arrays. When folders are added or removed: stop the current chat agent (`acp_agent_stop`), clear `acpAgent` state, so the next message send triggers a fresh spawn with the updated sandbox paths. Show a toast: "Agent restarted — workspace folders changed". Do NOT respawn on project selection changes in chat footer (handled by Layer 2 session isolation). Do NOT respawn the task agent (it uses single-folder sandbox anyway).
