...
- **Tasks done (108/129)** — 2026-04-24 live test returned findings: #117 (switch cards) and #119 (orb pulse scaling) **reverted ✅ → 🔁** because the user rejected the implementations as design-mismatches to what was approved. #117 rescoped to icon-only footer indicators (no disruptive cards); #119 rescoped to radiating-ring pulse per mockup. #106 (hover polish) and #125 (CommandBarContext parity) scopes extended with live-test findings. Two genuinely new tasks filed: #131 (remove DocHead breadcrumb, fold dirty+saved into TitleBar) and #132 (`uiPreview` toggle progress indicator). Small bugs to fix in-line (not new tasks): StatusTray popover anchor, LocalAI dot color states. **Currently in flight: none. Next up: #116 (send debug — blocking) first, then small in-line bug fixes, then #117-rescoped + #119-rescoped pivots, then #106/#131/#132 polish. Full queue now 21 pending.**
