...
- **Tasks done (99/117)** — Revised 2026-04-23 after user live-tested #102 and found five functional regressions in M1.2 (⌘K dead, double-tap ⌘ dead, send shows bubble with no response, AgentSwitchCard missing from stream, history unreachable) plus AgentOrb pulse invisible. **✅ reverted** on #20, #21, #23, #24, #27 (flipped to ⚠️ with references to the fix tasks). M1.2 drops from 21/21 → 16/21. New M1.12 tasks filed: #113 functional-parity audit (separate from #111 visual audit), #114 wire cmd-bar-events subscriber + Esc fall-through, #115 mount useDoubleTapCmd, #116 investigate send-no-response, #117 render AgentSwitchCard in CommandBarStream, #118 render history in FloatingCommandBar, #119 fix AgentOrb pulse CSS conflict, #120 focus-mode collapses command bar. Phase 1 is NOT shippable until these all land. M1.12 grew from 8 → 15 tasks. **Currently in flight: none. Next up: #113 parity audit first (produces the full gap inventory), then fan out #114–#120 fixes.**
