...
**PRD:** `docs/prds/2026-03-08-stability-persistence-observability.md`
**Total:** 22 tasks — 5S, 10M, 7L
...
**Description:**
Add `tauri-plugin-log` to `Cargo.toml` and `log` crate. Configure the plugin in `lib.rs` with:
...
**Description:**
Replace all `eprintln!` and `debug_log!` calls in the Rust backend with appropriate `log::*` macros (`log::info!`, `log::warn!`, `log::error!`, `log::debug!`). Use target-qualified logging for category filtering.
...
**Description:**
Create `src/lib/logger.ts` with `log.debug()`, `log.info()`, `log.warn()`, `log.error()` methods. Each method:
...
**Description:**
Replace all `debugLog()` calls and diagnostic `console.log()` calls with `log.*()` calls using appropriate categories and levels. Delete `src/lib/debug.ts`.
...
**Description:**
Add a "Log Files" section in Settings > Advanced, below the existing "Debug Logging" toggle:
...
**Description:**
Add `store_read`, `store_write`, and `store_delete` commands in a new `src-tauri/src/commands/store.rs`. Files stored in `~/.notesage/state/{key}.json`. Writes must be atomic: write to `.tmp` file, then `rename()`.
...
**Description:**
Create `src/lib/tauri-storage.ts` implementing Zustand's `StateStorage` interface:
...
**Description:**
Replace `{ name: 'store-name' }` persist config with `{ name: 'store-name', storage: createTauriStorage() }` in all persisted stores. The adapter's `getItem` must handle the async nature of Tauri IPC (Zustand persist supports async storage).
...
**Description:**
Add migration logic in the storage adapter or a startup hook:
...
**Description:**
Add pruning to `activity-store.ts`:
...
**Description:**
Cap `scrollPositions` map at 200 entries. On `setScrollPosition()`, if the map exceeds 200, remove the entry that was set least recently. Use insertion order (Object keys in modern JS are ordered by insertion) — delete the oldest key.
...
**Description:**
Split transient streaming fields out of persisted state:
...
**Description:**
Add two commands in a new `src-tauri/src/commands/health.rs`:
...
**Description:**
Add a `useEffect` in `App.tsx` that listens for `visibilitychange`. When the page becomes visible:
...
**Description:**
Wrap all blocking `read_line()` / `read_exact()` calls in reader loops with `tokio::time::timeout(Duration::from_secs(30), ...)`:
...
**Description:**
Add `NSSupportsAutomaticTermination = false` and `NSAppSleepDisabled = true` to the macOS Info.plist. In Tauri v2, this is configured via `src-tauri/Info.plist` or `tauri.conf.json` macOS section.
...
**Description:**
Replace the MutationObserver-based cleanup in `inline-diff.ts` with a deterministic approach:
...
**Description:**
Wrap the 50ms flush `setInterval` in `useAIOperations.ts` streaming path with a `try/finally` block that guarantees `clearInterval` runs on all code paths (success, error, cancellation). Add a final `flush()` call after clearing the interval.
...
**Description:**
In `useFileWatcher.ts`, add a size check before adding to `modifyDebounce.current` and `icloudDiscoveryDebounce.current`. If either map exceeds 500 entries:
